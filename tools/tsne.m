...
%                        accuracy. It is only supported in
%                        ExhaustiveSearcher.
%       'fastseuclidean'   
%                      - Standardized Euclidean distance computed by 
%                        using an alternative algorithm that saves 
%                        time. This faster algorithm can, in some 
%                        cases, reduce accuracy. It is only supported in
%                        ExhaustiveSearcher.
...
%                    requires an intermediate matrix (when 'Distance' is 
%                    one of {'fasteuclidean','fastseuclidean'}).
%                    If numeric, this argument specifies the cache size in 
%                    megabytes (MB) to allocate for an intermediate matrix. 
%                    If 'maximal', tsne attempts to allocate enough memory
%                    for an entire intermediate matrix whose size is N-by-N
%                    (N is the number of rows of the input data X).
%                    'CacheSize' does not have to be large enough for an 
%                    entire intermediate matrix, but must be at least large
%                    enough to hold an N-by-1 vector. Otherwise, the regular
%                    algorithm of computing Euclidean distance will be used
%                    instead. If the specified cache size exceeds the
%                    available memory, MATLAB issues an out-of-memory error.
%
%   Example:
%   load('FisherIris.mat')
%   rng('default');
...
[algorithm, distance, ydims, numPCA, ystart, perplexity, exaggeration, learnrate, theta,...
  standardize, numprint, options, verbose, cacheSize] = internal.stats.parseArgs(paramNames, defaults, varargin{:});
...
% Check CacheSize
if ischar(distance) && ismember(distance, {'fasteuclidean' 'fastseuclidean'})
    distance = statslib.internal.checkArgsForFastEuc(distance,issparse(X),cacheSize,size(X,1),class(X));
end
...
if strcmpi(algorithm,'exact')
    ...
    if N==1
        ...
        if ischar(distance) && ismember(distance, {'fasteuclidean' 'fastseuclidean'})
            tempDistMat = pdist(X,distance,'CacheSize',cacheSize);
        else
    ...
    if k==0
        ...
        if isa(ns,'ExhaustiveSearcher')
            cacheArg = {'CacheSize', cacheSize};
        else
            cacheArg = {};
        end
        knnidx = knnsearch_ns(X, Y, k, varargin); %[knnidx,D] = knnsearch(ns,X,'k',k,cacheArg{:});
...
emptyJointProb = isempty(colidx) || isempty(rowcnt) || all(rowcnt == 0) || isempty(probMatX);
outputNeeded = verbose>0 || haveOutputFcn;

% If Barnes-Hut algorithm is used,
%    and joint probability matrix is not empty,
%    and no output/printing is needed,
% find embedding by using builtin function.
if ~strcmpi(algorithm,'exact') && ~emptyJointProb && ~outputNeeded
    Ychange = zeros(Ydims,N,'like',Y);
    [Ytranspose,probMatX,Z] = internal.stats.tsnebhembedding( theta,Y',Ychange,...
        adpRatechange,colidx,rowcnt,probMatX,cumrowcnt,minRatechange,...
        momentums,momentumChange,exaggerationStop,k,phi,learnrate,...
        options.MaxIter,exaggeration,options.TolFun,sqrt(eps(class(Y))) );
    loss = internal.stats.tsnelossmex(Ytranspose,colidx,rowcnt,probMatX,Z);
    Y = Ytranspose';
else
